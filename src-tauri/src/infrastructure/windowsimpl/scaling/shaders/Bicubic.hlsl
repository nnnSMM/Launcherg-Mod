...
float4 Pass1(float2 pos) {
    // 疑似ボーダーレス化: クライアント領域のみにマッピング
    const float2 offset = GetSrcRectOffset();
    const float2 uvScale = GetScale(); // Client / Capture ratio
    pos = pos * uvScale + offset; // Map 0-1 to Client UV
