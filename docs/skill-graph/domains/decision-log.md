...
## 2026-06-06: Mobile Companionの補助は下部ナビの独立タブにする

- Context: `mode=controller` へ変更しても、スマホ画面の下部ナビが「ホーム/一覧/接続」のままだとユーザーには前と同じ体験に見える。詳細ページは補助の入口ではなく、情報確認とメモ編集の場所にしたい。
- Decision: 下部ナビを「ホーム/一覧/補助/接続」の4タブにし、補助画面中もナビを表示する。ゲーム詳細から補助CTAを置かず、詳細内のメモは常時編集可能なtextareaと同期ボタンにする。補助画面からクイックメモは外して、Pause/スクショ操作へ集約する。公開PWAの古いService Workerキャッシュを避けるため、cache nameも更新する。
- Rationale: 補助はアプリ全体の主要モードであり、個別ゲーム詳細の下位機能ではない。メモ編集を詳細に寄せることで、補助画面はプレイ中の少数操作だけに集中できる。
- Consequence: スマホから任意ゲームのメモは詳細ページで編集する。補助タブはPCで現在追跡中のゲームを検知できた時だけ操作可能になり、未検知時は待機表示になる。
- Links: [[mobile-companion-service-blueprint]], [[remote-play-hub]]
