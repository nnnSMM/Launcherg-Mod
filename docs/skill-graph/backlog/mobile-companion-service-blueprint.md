...
リモートスクリーンショットはスマホからPCへ「主ディスプレイ全体を撮る」命令を送り、起動中ゲームがPCアプリ側で特定できている場合だけ、通常のスクリーンショットDBへ登録する経路です。メモへMarkdown画像は挿入しない。ゲーム起動プロセスIDは対象ゲームが起動中として記録されているかの判定に使い、撮影自体は `save_screenshot_by_pid` ではなく主ディスプレイ全体の撮影にする。

- Controller MVP: 通常スクリーンショットDBへ登録し、撮影成功フィードバックと直前プレビューだけを検証する。スマホ側のGallery UIはまだ表示しない。
...
- Screenshot: PCの主ディスプレイ全体を撮り、起動中ゲームがPCアプリ側で特定できている場合だけ通常スクリーンショットDBへ登録する。メモへは自動追記しない。
...
- 通常スクショはメモ挿入ではなく、起動中ゲームに紐づくスクリーンショットDB/ギャラリーに保存される。
...
- スクリーンショットDB登録が漏れると、スマホで撮った画像がGalleryに出ない。メモ挿入経路へ戻さない。
