...
現行のリモートスクリーンショットは `save_screenshot_by_pid` で `game-memos` 配下に画像を保存し、メモへMarkdown画像を挿入する経路です。これはスクリーンショットDBへ登録する `import_screenshot` / `get_game_screenshots` 系のギャラリー経路とは同一ではない。
...
- Screenshot: 既存のメモ画像経路だけを使い、Capture Galleryとは接続しない。
