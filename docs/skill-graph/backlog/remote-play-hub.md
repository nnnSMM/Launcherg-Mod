...
- Screenshot: 起動中ゲームがPCアプリ側で特定できている場合だけ、主ディスプレイ全体を撮影して通常スクリーンショットDBへ登録する。メモへは自動追記しない。
...
- Screenshot: PCの主ディスプレイ全体を撮影し、起動中ゲームがPCアプリ側で特定できている場合だけ通常スクリーンショットDBへ登録する。メモへは自動追記しない。ゲーム起動プロセスIDは起動中判定に使い、撮影自体は `save_screenshot_by_pid` に依存しない。
...
- `startProcessMap` はフロント状態なので、再接続やリロードで失われうる。Mobile Companionの操作判定はRust側の追跡セッションを正とし、対象プロセス状態を確認できる応答を返す。
...
- Recover In Place: 接続切れ、撮影失敗、対象ゲーム未起動が起きても、PC画面に戻らずスマホ側で再接続や再試行へ進める。
...
- Risk: 中。現行SkyWayの信頼境界、スクリーンショットDB登録の取りこぼし、外部ツール依存、同一LAN検出で詰まりやすい。
...
- 撮影結果が起動中ゲームの通常スクリーンショットとしてDB登録され、メモへ自動追記されないこと。
