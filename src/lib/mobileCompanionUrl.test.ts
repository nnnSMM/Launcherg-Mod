...
describe("createMobileCompanionUrl", () => {
  it("points the QR to the mobile companion controller route", () => {
    ...
    expect(url).toBe(
      "https://example.test/companion.html?client=mobile-pwa-v4&mode=controller&roomId=room-1",
    );
  });
  ...
  it("keeps a subdirectory origin when building the companion entry URL", () => {
    ...
    expect(url).toBe(
      "https://example.test/Launcherg-Mod/companion.html?client=mobile-pwa-v4&mode=controller&roomId=room-1",
    );
  });
  ...
  it("can include an initial game without making the QR game-detail-only", () => {
    ...
    expect(url).toBe(
      "https://example.test/companion.html?client=mobile-pwa-v4&mode=controller&roomId=room-1&gameId=42&seiyaUrl=https%3A%2F%2Fseiya.example%2Fgame%3Fid%3D42&authToken=token-1",
    );
  });
  ...
  it("can explicitly create a library URL", () => {
    ...
    expect(url).toBe(
      "https://example.test/companion.html?client=mobile-pwa-v4&mode=library&roomId=room-1",
    );
  });
});
