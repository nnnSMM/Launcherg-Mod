...
WHEN dlsite_id IS NOT NULL THEN 'https://img.dlsite.jp/modpub/images2/work/doujin/' || left(dlsite_id,2) || LPAD(CAST(CAST(RIGHT(LEFT(dlsite_id, LENGTH(dlsite_id)-3), LENGTH(dlsite_id)-5) AS INTEGER) + 1 AS TEXT), LENGTH(dlsite_id)-5, '0') || '000/' || dlsite_id || '_img_main.jpg'WHEN dmm IS NOT NULL THEN 'https://pics.dmm.co.jp/mono/game/' || dmm || '/' || dmm || 'pl.jpg'
